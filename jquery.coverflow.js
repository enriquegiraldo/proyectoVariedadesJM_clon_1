    ...
    $.widget("vanderlee.coverflow", {
      ...
      _create: function () {
        ...
        that._window_handler_keydown = function (event) {
          if (
            ($.inArray(that.options.enableKeyboard, [true, "both", "focus"]) >=
              0 &&
              that.element.is(":focus")) ||
            ($.inArray(that.options.enableKeyboard, [true, "both", "hover"]) >=
              0 >=
              0 &&
              that.element.is(":hover"))
          ) {
            switch (event.which) {
              case 36: // home
                event.preventDefault();
      ...
      _setIndex: function (index, animate, initial) {
        ...
        if (index !== that.options.index) {
          ...
          if (animate === true || that.options.duration === 0) {
            ...
            var duration =
              typeof that.options.duration === "number"
                ? that.options.duration
                : jQuery.fx.speeds[that.options.duration] ||
      ...
      _frame: function (frame) {
        ...
        var that = this,
          ...
          coverWidth =
            that.options.width ||
            covers.eq(this.options.index).show().get(0).offsetWidth,
          visible =
            that.options.visible === "density"
              ? Math.round((parentWidth * that.options.density) / coverWidth)
              : $.isNumeric(that.options.visible)
              ? that.options.visible
              : count,
          parentLeft =
            that.element.position().left -
            (1 - that.options.outerScale) * coverWidth * 0.5,
          ...
          var $cover = $(cover),
            ...
            left =
              ...
              scl(
                ...
                0,
                1,
                that.options.innerOffset * that.options.density,
                space
              ),
            scale = isVisible
              ? scl(
                  ...
                  1,
                  0,
                  that.options.innerScale,
                  that.options.outerScale
                )
              : 0,
            angle =
              sign(sin) *
              scl(
                ...
                0,
                1,
                that.options.innerAngle,
                that.options.outerAngle
              ),
            css = isMiddle
              ? that.options.selectedCss || {}
              : $.interpolate &&
                that.options.outerCss &&
                !$.isEmptyObject(that.options.outerCss)
              ? isVisible
                ? $.interpolate(
                    that.options.innerCss || {},
                    that.options.outerCss,
                    Math.abs(sin)
                  )
                : that.options.outerCss
        ...
        that.element.stop().animate(
          ...
          },
          {
            ...
            step: function (now, fx) {
              ...
              that.currentIndex = Math.max(
                0,
                Math.min(Math.round(now), covercount - 1)
              );
